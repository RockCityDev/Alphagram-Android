...
public class MyDownloadIngRvAdapter extends BaseDelegateMultiAdapter<KKFileMessage, BaseViewHolder> implements LoadMoreModule {
    Context context;
    boolean edit;
    public List<String> selectList = new ArrayList<>();


    public MyDownloadIngRvAdapter(Context context) {
    ...
    private void initDelegate() {
        ...
        setMultiTypeDelegate(new BaseMultiTypeDelegate<KKFileMessage>() {
            ...
            public int getItemType(@NotNull List<? extends KKFileMessage> data, int position) {
                ...
                if (message.getFileType() == KKFileTypes.VIDEO_FILTER || message.getFileType() == KKFileTypes.MKV || message.getFileType() == KKFileTypes.MOV || message.getFileType() == KKFileTypes.MP4) {//媒体
                    return 1;
                } else {
                    return 2;
                }
    ...
    public void deleteItem(String downloadFileName) {
        int position = -1;
        for (int i = 0; i < getData().size(); i++) {
            KKFileMessage entity = getItem(i);
            if (downloadFileName.equals(entity.getDownloadFileName())) {
                position = i;
                break;
            }
        }
    ...
    public KKFileMessage getMessageByFileName(String fileName) {
        List<KKFileMessage> dataList = new ArrayList<>(getData());
        for (KKFileMessage kkVideoMessage : dataList) {
    ...
    private void showMediaItem(BaseViewHolder baseViewHolder, KKFileMessage entity) {
        ...
        if (selectList.contains(entity.getDownloadFileName())) {
        ...
        baseViewHolder.setText(R.id.tv_group_name, LocaleController.getString("video_from", R.string.video_from) + entity.getFromName());
        ...
        if (entity.getDownloadStatus().getStatus() == KKFileDownloadStatus.Status.DOWNLOADED) {//已完成
            ...
            tvDownloadState.setText(LocaleController.getString("download_nostart", R.string.download_nostart));
            tvDownloadSize.setText(SystemUtil.getSizeFormat(entity.getDownloadStatus().getTotalSize()));
            ...
            ivRight.setImageResource(R.drawable.ic_do_download);
            tvDownloadState.setVisibility(View.VISIBLE);
            tvDownloadState.setText(LocaleController.getString("download_pause", R.string.download_pause));
            ...
            ivRight.setImageResource(R.drawable.ic_do_download);
            tvDownloadState.setText(LocaleController.getString("download_fail", R.string.download_fail));
            progressBar.setProgress(pro);
            tvDownloadSize.setText(SystemUtil.getSizeFormat(entity.getDownloadStatus().getDownloadedSize()) + "/" + SystemUtil.getSizeFormat(entity.getDownloadStatus().getTotalSize()));
        }
    }
