 ...
 * Time:2022/8/8
 * Author:Perry
 * Description：黑白名单选择用户页面，所有的列表
 */
public class AllPermissionsChatAdp extends BaseQuickAdapter<PermissChatEntity, BaseViewHolder> {

    public AllPermissionsChatAdp() {
        super(R.layout.adp_permissions_chat);
    }

    @Override
    protected void convert(@NonNull BaseViewHolder viewHolder, PermissChatEntity data) {
        AdpPermissionsChatBinding binding = BaseViewHolderUtilKt.getBinding(viewHolder, AdpPermissionsChatBinding::bind);
        binding.tvRemove.setVisibility(View.GONE);
        binding.tvName.setText(data.getChatName());
        setHeaderImg(binding.flAvatar, data);

        //是否选中标识
        binding.ivSelector.setVisibility(data.isIfChecked() ? View.VISIBLE : View.GONE);
