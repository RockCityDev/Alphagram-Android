...
public class AboutLinkCell extends FrameLayout {
    ...
    //是否适配主题
    private boolean nofitTheme = true;
    // 自定义END
    ...
    public AboutLinkCell(Context context, BaseFragment fragment,boolean customTextX) {
        this(context, fragment, null);
        this.customTextX = customTextX;
    }
...
//            event.offsetLocation(showMoreTextBackgroundView.getLeft(), showMoreTextBackgroundView.getTop());
    ...
    private void drawText(Canvas canvas) {
        ...
        canvas.translate(textX = AndroidUtilities.dp(customTextX ? 18 : 23), 0);
    ...
    public void setTextAndValue(String text, String value, boolean parseLinks) {
        ...
        Emoji.replaceEmoji(stringBuilder, nofitTheme ? Theme.profile_aboutTextPaint.getFontMetricsInt() : getTextPaint().getFontMetricsInt(), AndroidUtilities.dp(20), false);
        if (lastMaxWidth <= 0) {
...
//            float speedMultiplier = Math.min(Math.max(dHeight / AndroidUtilities.dp(76), 0.5f), 2f);
    ...
    private void setHeight(int height) {
        ...
        if (lp instanceof RecyclerView.LayoutParams) {
            lp = (RecyclerView.LayoutParams) getLayoutParams();
        } else if (lp instanceof FrameLayout.LayoutParams) {
            lp = (FrameLayout.LayoutParams) getLayoutParams();
        }
        int wasHeight;
        boolean newHeight;
        if (lp == null) {
    ...
    private StaticLayout makeTextLayout(CharSequence string, int width) {
        if (Build.VERSION.SDK_INT >= 24) {
            return StaticLayout.Builder.obtain(string, 0, string.length(), nofitTheme ? Theme.profile_aboutTextPaint : getTextPaint(), width)
                    .setBreakStrategy(StaticLayout.BREAK_STRATEGY_HIGH_QUALITY)
                    .setHyphenationFrequency(StaticLayout.HYPHENATION_FREQUENCY_NONE)
                    .setAlignment(LocaleController.isRTL ? StaticLayoutEx.ALIGN_RIGHT() : StaticLayoutEx.ALIGN_LEFT())
                    .build();
        } else {
            return new StaticLayout(string, nofitTheme ? Theme.profile_aboutTextPaint : getTextPaint(), width, Layout.Alignment.ALIGN_NORMAL, 1.0f, 0.0f, false);
        }
    }
...
//    }
     ...
     * @param nofitTheme
     */
    public void setNofitTheme(boolean nofitTheme) {
        this.nofitTheme = nofitTheme;
        if (!nofitTheme) {
            showMoreBackgroundDrawable.setColorFilter(Color.WHITE, PorterDuff.Mode.MULTIPLY);
            showMoreTextBackgroundView.setBackground(showMoreBackgroundDrawable);
    ...
    private TextPaint getTextPaint() {
        if (textPaint == null) {
            ...
            textPaint.setColor(Color.parseColor("#1A1A1D"));
            textPaint.linkColor = ColorUtils.getColor(R.color.theme_color);
        }
        return textPaint;
    }
}
