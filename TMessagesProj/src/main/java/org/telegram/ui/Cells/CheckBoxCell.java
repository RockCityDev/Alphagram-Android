...
public class CheckBoxCell extends FrameLayout {
    ...
    private int rightImageSise = 50;
    private ImageView rightImageView;

    public CheckBoxCell(Context context, boolean customStyle) {
        this(context, 4, 17, null,customStyle);
    }
    public CheckBoxCell(Context context, int type) {
        this(context, type, 17, null,false);
    }

    public CheckBoxCell(Context context, int type, Theme.ResourcesProvider resourcesProvider) {
        this(context, type, 17, resourcesProvider,false);
    }


    public CheckBoxCell(Context context, int type, int padding, Theme.ResourcesProvider resourcesProvider, boolean customStyle) {
        super(context);
        ...
        addView(valueTextView, LayoutHelper.createFrame(LayoutHelper.WRAP_CONTENT, LayoutHelper.MATCH_PARENT, (LocaleController.isRTL ? Gravity.LEFT : Gravity.RIGHT) | Gravity.TOP, padding, 0, customStyle ? padding + rightImageSise : padding, 0));
        ...
        if (customStyle) {
            ...
            rightImageView.setImageResource(R.drawable.ic_arrow_right);
            //rightImageView.setScaleType(ImageView.ScaleType.FIT_CENTER);
            rightImageView.setPadding(AndroidUtilities.dp(25),AndroidUtilities.dp(16),0,AndroidUtilities.dp(16));
            addView(rightImageView, LayoutHelper.createFrame(rightImageSise, LayoutHelper.MATCH_PARENT, Gravity.RIGHT | Gravity.TOP, padding, 0, padding, 0));
        }
        updateTextColor();
    }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        if (currentType == 3) {
            ...
            checkBox.measure(MeasureSpec.makeMeasureSpec(AndroidUtilities.dp(checkBoxSize), MeasureSpec.AT_MOST), MeasureSpec.makeMeasureSpec(AndroidUtilities.dp(checkBoxSize), MeasureSpec.EXACTLY));
            if (customStyle) {
                rightImageView.measure(MeasureSpec.makeMeasureSpec(AndroidUtilities.dp(rightImageSise), MeasureSpec.EXACTLY), MeasureSpec.makeMeasureSpec(getMeasuredHeight(), MeasureSpec.EXACTLY));
            }
        }
    }
