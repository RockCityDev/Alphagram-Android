...
public class ProxyListActivity extends BaseFragment implements NotificationCenter.NotificationCenterDelegate {
    ...
    ProxyListActivity(Bundle bundle) {
        extendBundle = bundle;
    }
    ...
    public boolean onFragmentCreate() {
        ...
        if (SharedConfig.proxyList.size() == 0) {
            AndroidUtilities.runOnUIThread(() -> dealFrom(), 350);
        }
    ...
    private void dealFrom() {
        if (extendBundle != null && extendBundle.containsKey("from")) {
            String from = extendBundle.getString("from");
            if ("loginActivity".equals(from)) {
