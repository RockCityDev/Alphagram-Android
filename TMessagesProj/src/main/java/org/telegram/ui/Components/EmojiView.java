...
import teleblock.ui.adapter.GifStickerAdapter;
import teleblock.util.EventUtil;

public class EmojiView extends FrameLayout implements NotificationCenter.NotificationCenterDelegate {

    // 自定义START
    private RecyclerListView collectGridView;
    private GridLayoutManager collectLayoutManager;
    private GifStickerAdapter collectAdapter;
    private RecyclerListView.OnItemClickListener collectOnItemClickListener;
    // 自定义END

    private final static int TAB_EMOJI = 0;
    private final static int TAB_GIFS = 1;
    private final static int TAB_STICKERS = 2;
    private final static int TAB_COLLECTS = 3;
    ...
    private final int[] tabsMinusDy = new int[4];
    private ObjectAnimator[] tabsYAnimators = new ObjectAnimator[4];
    ...
    public void setAllow(boolean allowStickers, boolean allowGifs, boolean animated) {
        ...
        for (int i = 0; i < allTabs.size(); i++) {
            if (allTabs.get(i).type == TAB_EMOJI) {
                ...
                currentTabs.add(allTabs.get(i));
            } if (allTabs.get(i).type == TAB_COLLECTS) {
                currentTabs.add(allTabs.get(i));
            }
        }
        if (typeTabs != null) {
    ...
    @IntDef({Type.STICKERS, Type.EMOJIS, Type.GIFS, Type.COLLECTS})
    ...
    private class TypedScrollListener extends RecyclerListView.OnScrollListener {
        ...
        public void onScrollStateChanged(RecyclerView recyclerView, int newState) {
            if (type == Type.COLLECTS) return; // 防止引发触摸事件问题
...
//                }
    ...
    public EmojiView(BaseFragment fragment, boolean needAnimatedEmoji, boolean needStickers, boolean needGif, final Context context, boolean needSearch, final TLRPC.ChatFull chatFull, ViewGroup parentView, Theme.ResourcesProvider resourcesProvider) {
        ...
        tabIcons = new Drawable[]{
                ...
                Theme.createEmojiIconSelectorDrawable(context, R.drawable.smiles_tab_collect, getThemedColor(Theme.key_chat_emojiPanelBackspace), getThemedColor(Theme.key_chat_emojiPanelIconSelected))
        };
...
//                        }
        ...
        pager = new ViewPager(context) {
            ...
            public void setCurrentItem(int item, boolean smoothScroll) {
                if (searchButton != null) {
                    searchButton.setVisibility(item == 3 ? INVISIBLE : VISIBLE);
                }
                ...
                if (item == getCurrentItem()) {
                    if (item == 0) {
                        ...
                        collectGridView.smoothScrollToPosition(0);
                    }
                    return;
                }
        ...
        pager.addOnPageChangeListener(new ViewPager.OnPageChangeListener() {
            ...
            public void onPageScrolled(int position, float positionOffset, int positionOffsetPixels) {
                if(position == 3){
                    EventUtil.track(context, EventUtil.Even.表情包收藏界面展示,new HashMap<>());
                }
            }
...
//                        (SystemClock.elapsedRealtime() - touch.time) <= ViewConfiguration.getTapTimeout() * 1.2f &&
    ...
    private void addMyCollectView(Context context) {
        FrameLayout collectContainer = new FrameLayout(context);
        Tab collectTab = new Tab();
        collectTab.type = TAB_COLLECTS;
        collectTab.view = collectContainer;
        allTabs.add(collectTab);
        collectGridView = new RecyclerListViewWithOverlayDraw(context) {

            @Override
            public boolean onInterceptTouchEvent(MotionEvent event) {
                boolean result = ContentPreviewViewer.getInstance().onInterceptTouchEvent(event, collectGridView, EmojiView.this.getMeasuredHeight(), contentPreviewViewerDelegate, resourcesProvider);
                return super.onInterceptTouchEvent(event) || result;
            }
        };
        collectGridView.setLayoutManager(collectLayoutManager = new GridLayoutManager(context, 4));
        collectGridView.setBottomGlowOffset(AndroidUtilities.dp(44));
        collectGridView.setPadding(AndroidUtilities.dp(5), 0, AndroidUtilities.dp(5), AndroidUtilities.dp(44));
        collectGridView.setGlowColor(getThemedColor(Theme.key_chat_emojiPanelBackground));
        collectGridView.setClipToPadding(false);
        collectGridView.addItemDecoration(new RecyclerView.ItemDecoration() {
            int spacing = AndroidUtilities.dp(5);
        ...
        collectAdapter = new GifStickerAdapter(context);
        StickerSetNameCell cell = new StickerSetNameCell(getContext(), false, resourcesProvider);
        RecyclerView.LayoutParams lp = new RecyclerView.LayoutParams(LayoutHelper.MATCH_PARENT, LayoutHelper.WRAP_CONTENT);
        lp.setMargins(AndroidUtilities.dp(-10), AndroidUtilities.dp(5), 0, AndroidUtilities.dp(-2));
        cell.setLayoutParams(lp);
        cell.setText(LocaleController.getString("sticker_my_collect",R.string.sticker_my_collect), 0);
        collectAdapter.addHeaderView(cell);
        collectGridView.setAdapter(collectAdapter);
        collectContainer.addView(collectGridView, LayoutHelper.createFrame(LayoutHelper.MATCH_PARENT, LayoutHelper.MATCH_PARENT));
        collectGridView.setOnScrollListener(new TypedScrollListener(Type.COLLECTS));
        collectGridView.setOnTouchListener((v, event) -> ContentPreviewViewer.getInstance().onTouch(event, collectGridView, EmojiView.this.getMeasuredHeight(), collectOnItemClickListener, contentPreviewViewerDelegate, resourcesProvider));
        collectOnItemClickListener = (view, position) -> {
            EventUtil.track(getContext(), EventUtil.Even.表情包收藏界面发送, new HashMap<>());
            switch (collectAdapter.getItemViewType(position)) {
                case GifStickerEntity.TYPE_GIF:
                    ContextLinkCell linkCell = (ContextLinkCell) view;
                    delegate.onGifSelected(view, linkCell.getDocument(), null, "gif", true, 0);
                    break;
                case GifStickerEntity.TYPE_STICKER:
                    StickerEmojiCell cell1 = (StickerEmojiCell) view;
                    if (cell1.isDisabled()) {
                        return;
                    }
                    cell1.disable();
                    delegate.onStickerSelected(cell1, cell1.getSticker(), null, cell1.getParentObject(), cell1.getSendAnimationData(), true, 0);
                    break;
            }
        };
        collectGridView.setOnItemClickListener(collectOnItemClickListener);
    }
    ...
    private void checkGridVisibility(int position, float positionOffset) {
        ...
        if (position == 0) {
            ...
            collectGridView.setVisibility(GONE);
        } else if (position == 1) {
            ...
            stickersGridView.setVisibility(View.GONE);
            stickersTabContainer.setVisibility(View.GONE);
            collectGridView.setVisibility(VISIBLE);
        }
    }
...
//                    }
    ...
    public void onOpen(boolean forceEmoji) {
        onOpen(forceEmoji,currentPage);
    }

    public void onOpen(boolean forceEmoji,int currentPage) {
        ...
        if (currentPage == 0 || forceEmoji || currentTabs.size() == 1) {
            ...
            EventUtil.track(getContext(), EventUtil.Even.表情包输入框星星点击,new HashMap<>());
            showBackspaceButton(false, false);
            showStickerSettingsButton(false, false);
    ...
    protected void onAttachedToWindow() {
        ...
        if (collectAdapter != null) {
            NotificationCenter.getInstance(currentAccount).addObserver(this, NotificationCenter.loadGifStickerData);
        }
    }
    ...
    public void onDestroy() {
        ...
        if (collectAdapter != null) {
            NotificationCenter.getInstance(currentAccount).removeObserver(this, NotificationCenter.loadGifStickerData);
        }
    }
    ...
    public void didReceivedNotification(int id, int account, Object... args) {
        if (id == NotificationCenter.stickersDidLoad) {
            ...
            updateEmojiHeaders();
            updateStickerTabs(false);
        } else if (id == NotificationCenter.loadGifStickerData) {
            if (collectAdapter != null) {
                collectAdapter.loadData();
            }
        }
    }
