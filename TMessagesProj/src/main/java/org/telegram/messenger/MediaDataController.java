...
public class MediaDataController extends BaseController {
    ...
    public void addRecentSticker(int type, Object parentObject, TLRPC.Document document, int date, boolean remove) {
        ...
        if (type == TYPE_FAVE) {
            if (remove) {
                StickerManager.getInstance().deleteGifStickerCollect(document);
                ...
                StickerManager.getInstance().collectGifSticker(document,3);
...
//        TLRPC.StickerSet stickerSet = messages_stickerSet.set;
    ...
    public void searchMessagesInChat(String query, long dialogId, long mergeDialogId, int guid, int direction, int replyMessageId, boolean internal, TLRPC.User user, TLRPC.Chat chat, boolean jumpToMessage) {
        ...
        boolean finalFirstQuery = firstQuery;
        reqId = getConnectionsManager().sendRequest(req, (response, error) -> {
            ArrayList<MessageObject> messageObjects = new ArrayList<>();
            Log.d("TTT","firstQuery = "+finalFirstQuery);
