...
public class MessagesController extends BaseController implements NotificationCenter.NotificationCenterDelegate {
    ...
    public MessagesController(int num) {
        ...
        maxPinnedDialogsCount = mainPreferences.getInt("maxPinnedDialogsCount2", 100);
        maxFolderPinnedDialogsCount = mainPreferences.getInt("maxFolderPinnedDialogsCount2", 100);
        ...
        dialogFiltersPinnedLimitDefault = mainPreferences.getInt("dialogFiltersPinnedLimitDefault2", 100);
        dialogFiltersPinnedLimitPremium = mainPreferences.getInt("dialogFiltersPinnedLimitPremium2", 100);
...
//            maxFolderPinnedDialogsCount = config.pinned_infolder_count_max;
    ...
    public void loadFullChat(long chatId, int classGuid, boolean force) {
        ...
        int reqId = getConnectionsManager().sendRequest(request, (response, error) -> {
            if (error == null) {
                ...
                Map<Long, Integer> memberCounts = MMKVUtil.getMemberCounts();
                memberCounts.put(chatId, res.full_chat.participants_count);
                MMKVUtil.setMemberCounts(memberCounts);
                Timber.i("res.full_chat.id-->"+ chatId);
    ...
    public ArrayList<TLRPC.Dialog> getAllDialogs() {
        Timber.i("getAllDialogs-->" + allDialogs.size());
        return allDialogs;
    }
...
//                            if (newMsg == null && oldMs.getId() > 0 || newMsg != null && newMsg.messageOwner.date > oldMsg.messageOwner.date)
                ...
                Timber.i("allDialogs-->" + allDialogs.size());
    ...
    private void completeReadTask(ReadTask task) {
        if (task.replyId != 0) {
            ...
            if (inputPeer instanceof TLRPC.TL_inputPeerChannel) {
                ...
                int read_message = KKVideoMessageDB.getInstance(UserConfig.selectedAccount).queryContactRead(inputPeer.user_id);
                if (read_message == -1 && MMKVUtil.isOpenStealthMode()) {
                    return; // 未设置过 全局开关控制
                } else if (read_message == 1) {
                    return; // 已开启 不调接口
                }
            ...
            int read_message = KKVideoMessageDB.getInstance(UserConfig.selectedAccount).queryContactRead(chat.user_id);
            if (read_message == -1 && MMKVUtil.isOpenStealthMode()) {
                return; // 未设置过 全局开关控制
            } else if (read_message == 1) {
                return; // 已开启 不调接口
            ...
            return getConnectionsManager().sendRequest(req, (response, error) -> {
                ...
                AndroidUtilities.runOnUIThread(() -> {
                    ...
                    if (updates.chats != null && !updates.chats.isEmpty()) {
                        getNotificationCenter().postNotificationName(NotificationCenter.chatDidCreated, updates.chats.get(0).id);//创建新群提交相关数据
                    } else {
                        getNotificationCenter().postNotificationName(NotificationCenter.chatDidFailCreate);
                    }
                });
            ...
            if (response != null) {
                ...
                getMessagesStorage().getStorageQueue().postRunnable(() -> AndroidUtilities.runOnUIThread(() -> {
                    ...
                    for (int a = 0; a < dialogs.size(); a++) {
                        ...
                        // 服务端返回五条以上才合并本地置顶
                        if (folderId == 0 && newPinnedDialogs.size() >= 5) {
                            boolean needAdd = false;
                            for (TLRPC.Dialog newPinnedDialog : newPinnedDialogs) {
                                if (newPinnedDialog.id == dialog.id) {
                                    needAdd = false;
                                    break;
                                }
                                needAdd = true;
    ...
    public void sortDialogs(LongSparseArray<TLRPC.Chat> chatsDict) {
        sortDialogs(chatsDict, selectedDialogFilter);
    }

    public void sortDialogs(LongSparseArray<TLRPC.Chat> chatsDict, MessagesController.DialogFilter[] selectedDialogFilter) {
        dialogsServerOnly.clear();
        dialogsCanAddUsers.clear();
        dialogsMyGroups.clear();
        dialogsMyChannels.clear();
