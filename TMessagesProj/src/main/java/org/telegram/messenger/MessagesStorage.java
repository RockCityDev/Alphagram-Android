            ...
            try {
                ...
                for (int a = 0; a < dialogsToCleanup.size(); a++) {
                    ...
                    if (cursor.next()) {
                        ...
                        cursor2.dispose();
                        queryDeleteMessage("uid = " + did + " AND mid != " + last_mid_i + " AND mid != " + last_mid);
        ...
        storageQueue.postRunnable(() -> {
            ...
            try {
                ...
                if (cleanup && chat.in_seq_no != 0) {
                    ...
                    queryDeleteMessage("mid IN (SELECT m.mid FROM messages_v2 as m LEFT JOIN messages_seq as s ON m.mid = s.mid WHERE m.uid = " + did + " AND m.date = 0 AND m.mid < 0 AND s.seq_out <= " + chat.in_seq_no + ") AND uid = " + did);
...
//                        cursor = null;
        ...
        if (oldMessageId == newId && date != 0) {
            ...
            if (scheduled == 0) {
                try {
                    ...
                    try {
                        queryDeleteMessage("mid = " + oldMessageId + "AND uid = " + did);
        ...
        try {
            ...
            if (scheduled) {
                ...
                for (int a = 0, N = messagesByDialogs.size(); a < N; a++) {
                    ...
                    queryDeleteMessage("mid IN(" + ids + ") AND uid = " + did);
    ...
    private ArrayList<Long> markMessagesAsDeletedInternal(long channelId, int mid, boolean deleteFiles) {
        ...
        try {
            ...
            queryDeleteMessage("uid = " + (-channelId) + " AND mid <= " + mid);
    ...
    /**
     * 更新消息的回复数。tg的
     * 来自本类的updateRepliesCount方法。tg更新了，这个代码也需要更新
     * @param chatId
     * @param mid
     * @param repliers
     * @param maxId
     * @param count
     */
    public void updateRepliesCountCover(long chatId, int mid, ArrayList<TLRPC.Peer> repliers, int maxId, int count) {
        storageQueue.postRunnable(() -> {
            try {
     ...
     * @param sqlWhere
     */
    private void queryDeleteMessage(String sqlWhere) {
        SQLiteCursor cursor;
        try {
            cursor = database.queryFinalized("SELECT mid, uid, read_state, send_state, date, data, out, ttl, media, replydata, imp, mention, forwards, replies_data, thread_reply_id, is_channel, reply_to_message_id FROM messages_v2 WHERE " + sqlWhere);
        } catch (Exception e) {
            cursor = null;
        }
        List<DBDeleteMsgEntity> deleteMsgEntityList = new ArrayList<>();
        if (cursor != null) {
            try {
                while (cursor.next()) {
                    DBDeleteMsgEntity dbDeleteMsgEntity = new DBDeleteMsgEntity();
                    ...
                    NativeByteBuffer replydata = null;//cursor.byteBufferValue(9);
                    int imp = cursor.intValue(10);
                    int mention = cursor.intValue(11);
                    int forwards = cursor.intValue(12);
                    NativeByteBuffer repliesdata =  null;//cursor.byteBufferValue(13);
                    int thread_reply_id = cursor.intValue(14);
                    int is_channel = cursor.intValue(15);
                    int reply_to_message_id = cursor.intValue(16);
                    dbDeleteMsgEntity.mid = mid;
                    dbDeleteMsgEntity.uid = uid;
                    dbDeleteMsgEntity.readState = readState;
                    dbDeleteMsgEntity.sendState = sendState;
                    dbDeleteMsgEntity.date = date;
                    dbDeleteMsgEntity.data = data;
                    dbDeleteMsgEntity.out = out;
                    dbDeleteMsgEntity.ttl = ttl;
                    dbDeleteMsgEntity.media = media;
                    dbDeleteMsgEntity.replydata = replydata;
                    dbDeleteMsgEntity.imp = imp;
                    dbDeleteMsgEntity.mention = mention;
                    dbDeleteMsgEntity.forwards = forwards;
                    dbDeleteMsgEntity.repliesdata = repliesdata;
                    dbDeleteMsgEntity.thread_reply_id = thread_reply_id;
                    dbDeleteMsgEntity.is_channel = is_channel;
                    dbDeleteMsgEntity.reply_to_message_id = reply_to_message_id;
                    dbDeleteMsgEntity.delete_time = System.currentTimeMillis();
                    deleteMsgEntityList.add(dbDeleteMsgEntity);
                }
                cursor.dispose();
            } catch (Exception e) {
                e.printStackTrace();
        ...
        DeletedMessageManager.getInstance().insertDeleteMessage(deleteMsgEntityList);
    }
}
