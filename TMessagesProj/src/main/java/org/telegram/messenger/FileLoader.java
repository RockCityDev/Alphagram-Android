...
public class FileLoader extends BaseController {
    ...
    public static File getTelegramDirectory() {
        File dir = getDirectory(FileLoader.MEDIA_DIR_VIDEO);
        if (dir != null) return dir.getParentFile();
        File cache = getDirectory(FileLoader.MEDIA_DIR_CACHE);
...
//            }
    ...
    private FileLoadOperation loadFileInternal(final TLRPC.Document document, final SecureDocument secureDocument, final WebFile webDocument, TLRPC.TL_fileLocationToBeDeprecated location, final ImageLocation imageLocation, Object parentObject, final String locationExt, final long locationSize, int priority, final FileLoadOperationStream stream, final long streamOffset, boolean streamPriority, final int cacheType) {
        ...
        if (document != null && parentObject instanceof MessageObject) {
            KKLocalFileManager.getInstance().watchMessageObject((MessageObject) parentObject);
        }
        return operation;
    }
    ...
    public static String getDocumentSuffix(TLRPC.Document document) {
        String fileName = getDocumentFileName(document);
        int idx = fileName.lastIndexOf('.');
        String ext = null;
        if (idx != -1) {
