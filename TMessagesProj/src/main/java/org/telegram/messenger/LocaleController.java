...
public class LocaleController {
    ...
    public LocaleController() {
        ...
        localeInfo.nameEnglish = "Hong Kong";
    ...
    private void addLanguageFile(Locale newLocale) {
        String localeStr = newLocale.toString().toLowerCase();
        String path = PathUtils.getInternalAppFilesPath() + "/language/" + localeStr + "/string_ex.xml";
//        if (!FileUtils.createOrExistsFile(path) || FileUtils.getFileLength(path) == 0) {
            ResourceUtils.copyFileFromAssets("language/" + localeStr + "/string_ex.xml", path);
//        }
        Timber.i("localeValues-->" + localeValues.size());
        localeValues.putAll(getLocaleFileStrings(new File(path)));
        Timber.i("localeValues-->" + localeValues.size());
    }
